...
async def get_data(url, headers):
    ...
    async with aiohttp.ClientSession() as session:
        async with session.get(url=url, headers=headers) as response:
            ...
            for catalog_url in catalogs_urls:
                async with session.get(url=catalog_url, headers=headers) as cat_response:
                    cat_soup = BeautifulSoup(await cat_response.text(), "lxml")
                    ...
                    for page in range(1, pages + 1):
                        ...
                        async with session.get(url=products_catalog_url, headers=headers) as products_catalog_response:
                            products_catalog_soup = BeautifulSoup(await products_catalog_response.text(), "lxml")
                            ...
                            for product in products:
                                try:
                                    ...
                                    name = "Нет названия"
                                try:
                                    price = int("".join([symbol for symbol in product.find("div", class_="summary").text.strip() if symbol.isdigit()]))
                                except:
                                    price = "Нет цены"
                                try:
    ...
    async with aiofiles.open(r"D:\testsforpy\test1.json", "w+", encoding="utf-8") as file:
        json.dump(products_data, file, indent=4, ensure_ascii=False)

async def main(url, headers):
    await get_data(url, headers)
    print("Finish")
